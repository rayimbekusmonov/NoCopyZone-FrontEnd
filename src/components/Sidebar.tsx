...
const Sidebar: React.FC = () => {
  ...
  return (
    <motion.aside initial={{ x: -80, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="w-64 min-h-screen bg-slate-900 border-r border-slate-800 flex flex-col">
      ...
      <div className="p-4 mx-3 mt-4 rounded-xl bg-slate-800/50 border border-slate-700/50">
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-full flex items-center justify-center ${isAdmin ? 'bg-amber-500/20' : 'bg-indigo-500/20'}`}>
            <RoleIcon size={16} className={isAdmin ? 'text-amber-400' : 'text-indigo-400'} />
          </div>
        ...
        <div className="space-y-1">
          {links.map(({ to, icon: Icon, label, end }) => (
            <NavLink key={to} to={to} end={end}
              className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl transition font-medium text-sm ${
                isActive ? 'bg-indigo-500/15 text-indigo-400 border border-indigo-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}>
              <Icon size={18} /> {label}
            </NavLink>
          ))}
